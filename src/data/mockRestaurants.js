...
export function searchRestaurants(query, userLat = null, userLng = null) {
  ...
  let results = restaurants.filter((r) => {
    ...
    const text = searchFields.filter(Boolean).join(" ").toLowerCase();
  ...
  if (userLat && userLng) {
    results = results
      .map((r) => ({
        ...r,
        distance: calcDistance(userLat, userLng, r.y, r.x),
      }))
...
export function getNearbyRestaurants(
  ...
  return restaurants
    ...
    .filter((r) => r.distance <= maxDist)
    .sort((a, b) => a.distance - b.distance)
    .slice(0, limit);
}

// 기존 데이터 유지 (다른 페이지에서 사용 중)
